...
yn = c('OpT','NOpT','VT')                       # response variables
...
ForecastResults = ForecastList(yn, nrow(OEEbw), c(paste0('q',LAGS),'persistence'))
...
for (wk in 1:length(weeks)) {
  ...
  for (ii in 1:length(LAGS)) {
    ...
    message(paste('*** Training parameters in model with',q,'lags ... ***\n'))
  ...
  for (ii in 1:length(LAGS)) {
    ...
    for (i in 1:L) {
      j = i + Fi
      if (i%%100==0) cat(paste('****** Prediction #', j,' ******\n'))
      ...
      for (jj in 1:m1) {
        ...
        ForecastResults[[qname]][[v]][j, 'lower']      = yhat-1.96*shat
        ForecastResults[[qname]][[v]][j, 'upper']      = yhat+1.96*shat
...
#save(mtf_mv_forecast, file="results/MTF_mv_forecast.RData")
